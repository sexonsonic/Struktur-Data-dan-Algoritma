...
{
    // setiap node dijadikan root/parent
...
void Union(struct subset subsets[], int x, int y)
{
    ...
    // tambah rank tree dibawah parent
    if (subsets[xroot].rank < subsets[yroot].rank)
        subsets[xroot].parent = yroot;
    else if (subsets[xroot].rank < subsets[yroot].rank)
        subsets[xroot].parent = xroot;
    // jika nilai bobot sama, maka increament rank ++
    else
    {
...
void kruskalMST(struct Graph *graph)
{
    int V = graph->V;
    struct Edge hasil[V];
    int e = 0; // index untuk menghitung hasil
    int i = 0; // index untuk menghitung sorting edge

    // 1. Sorting semua edge mulai dari yang terkecil sampai nilai terbesar
    qsort(graph->edge, graph->E, sizeof(graph->edge[0]), myComp);

    // Alokasi Memory
    struct subset *subsets = new subset;
    // Menjadi single element
    for (int v = 0; v < V; v++)
    {
        subsets[v].parent = v;
        subsets[v].rank = 0;
    ...
    // 2. Melakukan Looping hingga V - 1
    while (e < V - 1)
    {
    ...
    cout << "Edge yang terbentuk menjadi MST" << endl;
    int total;
    for (i = 0; i < e; i++)
    {
